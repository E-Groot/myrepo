...
ggplotly(gg, height=680, width=950)
...
gg2 <- ggplot(corpus, aes(x = speechiness, y = danceability, color = new_year, text = paste(track.name))) +
  ...
  scale_y_continuous(limits = c(0.4, 1)) + theme_bw() +theme(
          ...
          legend.title = element_text(size = 12),
          legend.text = element_text(size = 10)) 
ggplotly(gg2, tooltip = c("x", "y", "text"), hoverinfo = "x+y+text", height=680, width=950)
```
...
corpus <- corpus %>%
  mutate(new_year = case_when(
    track.album.release_date < 2000 ~ "90s",
    track.album.release_date >= 2000 ~ "modern"
  ))
...
ggplotly(Loudness, height=680, width=950)
```
...
Column {data-width=350} 
-----------------------------------------------------------------------
### Conclusions and assumptions based on the mean tempo and standard deviation
we can see that the standard deviation overall is very low, this means that there is not a lot of variance in the calculated means of tempo; something we would expect with hiphop and rap music from both these periods, as the beats and rhythm usually are very repetitive and steady. It is a logical conclusion that this also explains why this music on average has such a high danceability, as we saw earlier. Besides the fact that this graph shows the BPM's from this corpus as really steady with a low standard deviation, we can see a clear difference between the oldschool hiphop from the 90's and the modern rap music. As the oldschool hiphop music (apart from the 3 outliers on the right) has significantly lower mean tempo than the modern rap music. One interesting observation, something we saw earlier in the valence and energy graph too, is that the modern rap music shows a lot of fluctuation in values compared to the 90's period. We can see a steady cluster of some points around 140/150 BPM but also a lot of different BPM's, where the oldschool really has a steady cluster off all the points, besides the 3 outliers with high BPM, from around 80 to roughly 115 BPM


One of the most streamed songs on Spotify (chromagram) {data-navmenu="Further Visualisation and Analysis"}
=======================================
Column
-----------------------------------------------------------------------
### Looking at one of the outliers: "rockstar (feat. 21 Savage)" 
In the graph about energy and valence we saw some remarkable outliers, one of them being the song "rockstar" from Post Malone featuring 21Savage which consists of the lowest valence of the whole corpus. This song is also the most streamed song of the corpus having above 2.7 billion streams and even is part of the top 5 most streamed songs on Spotify! Let's take a look at the chroma of this song in the forms of a chromagram and chordogram. A chromagram is used to identify the dominant pitches in a song. It contains all the 12 pitch classes (tone height is a irrelevant factor for chroma) from the Western tonal system. Herefore it is a very usable tool to represent and analyse an audio-file, as it gives us a lot of insight about the harmony and movement in the track. Besides the chromagram there is a chordogram to the right; this also uses chroma, but with some computational magic, uses the pitches from the song to estimate played chords. The chroma from this song is actually really interesting as it conveys some music theoretical contradictions that are counterintuitive on first sight. When looking at the chromagram we see two dominant notes besides the C in the intro; the notes C#/Db and G are repeated regularly through out the song and could be seen as the most abundant notes from the chromagram. Besides these two notes we also see some repetitive magnitude in other notes, which at times even show some chromaticism. An interesting nuance to this whole story thus far is that the SpotifyAPI concludes that this song is in F minor (key=5 and mode=0, for pratical reasons I'm not diving too deep in why these particular key and mode values mean this) while we see that the note F throughout the song actually is one of the few notes that has so little magnitude apart from the ending. One logical explaination for this would be that both a C#/Db key or G key dont share the two notes (C#/Db and G), either minor or major. 
